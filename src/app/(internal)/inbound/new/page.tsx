...
export default function NewInboundOrderPage() {
  ...
  // Convert entered qty to base units
  const getBaseUnits = (item: OrderItem): number => {
    const upc = item.units_per_case || getProductInfo(item.product_id)?.units_per_case || 1;
  ...
  const validate = (): boolean => {
    ...
    const invalidItems = items.filter(
      (item) => !item.product_id || item.qty_entered < 1
    );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const itemsData: CreateInboundItemData[] = items.map((item) => ({
        ...
        units_per_case: item.units_per_case ? Number(item.units_per_case) : null,
      }));
  ...
  const totalUnits = items.reduce((sum, item) => sum + getBaseUnits(item), 0);
  const totalPallets = items.reduce((sum, item) => sum + (item.pallet_count ? Number(item.pallet_count) : 0), 0);
              ...
              {items.length === 0 ? (
                ...
                <div className="space-y-3">
                  {items.map((item) => {
                    ...
                    const upc = item.units_per_case || product?.units_per_case || 1;
                    const baseUnits = getBaseUnits(item);
                      ...
                      <div key={item.id} className="border border-gray-200 rounded-lg p-4 space-y-3">
                        ...
                        <div className="grid grid-cols-4 gap-3">
                          <Input
                            ...
                            onChange={(e) =>
                              updateItem(item.id, "qty_entered", parseInt(e.target.value) || 0)
                            }
                          />
                          <Select
                            label="UOM"
                          ...
                          {item.uom === "cases" && (
                            <Input
                              ...
                              onChange={(e) =>
                                updateItem(item.id, "units_per_case", e.target.value === "" ? "" : parseInt(e.target.value) || 0)
                              }
                              placeholder={`${product?.units_per_case || 1}`}
                            />
                          )}
                          <Input
                            label="Pallets"
                            name={`pallets-${item.id}`}
                            type="number"
                            min={0}
                            value={item.pallet_count}
                            onChange={(e) =>
                              updateItem(item.id, "pallet_count", e.target.value === "" ? "" : parseInt(e.target.value) || 0)
                            }
                            placeholder="0"
                          />
                        </div>
